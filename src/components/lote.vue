  ...
  methods: {
    ...
    validar() {
      ...
      if (this.nombre.length < 1 || this.nombre.length > 50) {
        this.validaMensaje.push(
          "El nombre del lote debe tener entre 1-50 caracteres."
        );
      }else if (this.tipoAccion == 1){

        this.lotes.forEach(value => {
      ...
      if(this.descripcion.length < 1){
        this.validaMensaje.push(
        "La descripción del lote debe tener entre 1-255 caracteres."
        );
      }
      else if (this.descripcion.length > 255) {
        this.validaMensaje.push(
          "La descripción del lote no debe tener más de 255 caracteres."
        );
      }
    ...
    guardar() {
      ...
      if (this.editedIndex > -1) {
           db.collection('lotes').doc(me.key)
                .update({
      ...
      else
      {
        ...
        const lote = {
         ...
         this.$store.dispatch("addLote", lote); 
            me.limpiar();
            me.close();
            me.listar();
      }
